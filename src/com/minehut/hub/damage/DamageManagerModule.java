 ...
 */
public class DamageManagerModule implements Listener {
	...
	public void onVanillaDamage(EntityDamageEvent event) {
		...
		Hub.getInstance().getServer().getPluginManager().callEvent(damageEvent);
	...
	public void onRespawn(PlayerRespawnEvent event) {
		CustomRespawnEvent respawn = new CustomRespawnEvent(event.getPlayer(), Bukkit.getServer().getWorlds().get(0).getSpawnLocation());
		Hub.getInstance().getServer().getPluginManager().callEvent(respawn);
	...
	public void cancelDeath(CustomDamageEvent event) {
		...
		if ((((Damageable)event.getHurtEntity()).getHealth() - event.getDamage()) <= 0D) {
			...
			Hub.getInstance().getServer().getPluginManager().callEvent(new CustomDeathEvent(event));
			event.setCancelled(true);

			CustomRespawnEvent respawn = new CustomRespawnEvent(event.getHurtPlayer(), Bukkit.getServer().getWorlds().get(0).getSpawnLocation());
			Hub.getInstance().getServer().getPluginManager().callEvent(respawn);
